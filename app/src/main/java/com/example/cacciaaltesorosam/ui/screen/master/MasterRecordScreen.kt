...
fun MasterRecordScreen(
    ...
    val posizioneTesto = when {
        isLoading -> "CARICAMENTO..."
        masterCoord == null -> "NON FISSATA"
        else -> "%.5f, %.5f".format(masterCoord!!.latitude, masterCoord!!.longitude)
    }
    val posizioneColore = when {
        isLoading -> PixelYellow
    ...
    Column(Modifier
            ...
            )
        {
            ...
            Row {
                Text("POSIZIONE: ")
                Text(posizioneTesto, color = posizioneColore)
            }
